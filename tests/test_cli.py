...
from camoufox_cli.models import OkResponse, ResponseData, TitleCommand
...
class TestPrintResponseTabs:
    def test_renders_table_for_tabs_only_response(self, capsys):
        resp = OkResponse(
            id="r1",
            data=ResponseData(tabs=[{"index": 0, "url": "https://a", "title": "A", "active": True}]),
        )
...
class TestSendCommandRetrySafety:
    ...
    def test_truncated_reply_raises_response_error_without_re_execution(self):
        ...
        with pytest.raises(ResponseError):
            send_command(sock_path, TitleCommand(id="r1"))
    ...
    def test_connect_failure_is_not_response_error(self):
        ...
        with pytest.raises(Exception) as exc:
            send_command(missing, TitleCommand(id="r1"))
