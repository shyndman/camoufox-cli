...
# Tests cover the internal proxy-URL rebuilder directly; it is module-private
# by design (an implementation helper, not public API).
...
class TestProxyUrlWithAuth:
    ...
    def test_special_chars_round_trip(self, username: str, password: str):
        proxy_url = (
            f"http://{quote(username, safe='')}:"
            + f"{quote(password, safe='')}@127.0.0.1:8080"
        )
